...
#
# Beschreibung:
#   Gemeinsame Kern-Funktionen f�r alle Module
...
#
# -----------------------------------------------------------------------------
# Dependencies: liblogging, libfolders (f�r Core-Funktionen)
# -----------------------------------------------------------------------------
# Author: D.G�tze
# Version: 1.3.0
...
# ---------------------------------------------------------------------------
# Funktion.: Pr�fe alle Framework Abh�ngigkeiten (Modul-Dateien, die Modul
# .........  Ausgabe Ordner, kritische und optionale Software f�r die
# .........  Ausf�hrung des Tool), l�dt bei erfolgreicher Pr�fung die
# .........  Sprachdatei f�r das Modul.
# Parameter: keine
...
common_check_dependencies() {
    ...
    if declare -f folders_get_modul_output_dir >/dev/null 2>&1; then
        if ! folders_get_modul_output_dir "data" >/dev/null 2>&1; then
            log_error "$MSG_ERROR_OUTPUT_DIR_NOT_EXISTS $(folders_get_output_dir 2>/dev/null || echo '<unknown>')"
            return 1
        fi
    fi
...
# .........  Kopiermodul (Audio-CD, DVD-Video, Blu-ray Video). W�hlt die 
# .........  beste verf�gbare Methode basierend auf Fehler-Historie.
...
# ===========================================================================
common_copy_data_disc() {
    #-- Pr�fe Disc-Typ: Audio-CDs k�nnen nicht als ISO kopiert werden -------
    ...
    #-- Pr�fe ob diese Disc bereits fehlgeschlagen ist ----------------------
    local failure_count=$(common_get_disc_failure_count)
    
    #-- Pr�fe ob ddrescue vorhanden, es ist optional ------------------------
    ...
    if common_copy_data_disc_dd; then
        #-- Erfolg - l�sche Fehler-Historie falls vorhanden -----------------
...
# .........  Erstellt Map-Datei f�r Wiederherstellung bei Abbruch
...
common_copy_data_disc_ddrescue() {
    ...
    #-- Lese aus DISC_INFO Array die ben�tigten Werte -----------------------
    ...
    #-- ddrescue ben�tigt Map-Datei (im .temp Ordner, wird auto-gel�scht) ---
    local mapfile="${temp_pathname}/$(basename "${iso_filename}").mapfile"
    
    #-- Speicherplatz Pr�fung (falls Gr��e bekannt) -------------------------    
    if [[ $size_mb -gt 0 ]]; then
        #-- Logge erkannte Disc-Gr��e ---------------------------------------
        ...
        #-- Pr�fe Speicherplatz mit vorberechneter Gr��e (inkl. Overhead) ---
        if ! systeminfo_check_disk_space "$(discinfo_get_estimated_size_mb)"; then
            return 1
        fi
    fi
    
    #-- Starte ddrescue im Hintergrund (mit oder ohne Gr��enbeschr�nkung) ---
    ...
    #-- �berwache Fortschritt (alle 60 Sekunden) ----------------------------
...
# ---------------------------------------------------------------------------
# Funktion.: Kopiert Daten-Discs mit dd (Fallback-Methode, immer verf�gbar)
# .........  Nutzt vorberechnete DISC_INFO-Werte und zeitgesteuertes
# .........  Fortschritts-Monitoring (alle 60 Sekunden).
# Parameter: keine (nutzt DISC_INFO Array)
# R�ckgabe.: 0 = Erfolg
# .........  1 = Fehler (Speicherplatz, Kopiervorgang fehlgeschlagen)
# Extras...: Langsamste Methode, aber immer verf�gbar (keine Abh�ngigkeiten)
# .........  Unterst�tzt Kopieren mit/ohne Gr��enangabe
# .........  Sendet Fortschritt via API, MQTT und systemd-notify
# ===========================================================================
common_copy_data_disc_dd() {
    #-- Initialisiere Kopiervorgang-Log -------------------------------------
...
# ============================================================================
# FEHLER-TRACKING SYSTEM (f�r alle Disc-Typen)
# ============================================================================

# ===========================================================================
# common_get_disc_failure_count
# ---------------------------------------------------------------------------
# Funktion.: Pr�ft ob Disc bereits fehlgeschlagen ist und gibt die Anzahl
# .........  der bisherigen Fehlversuche zur�ck.
# Parameter: $1 = Disc-Identifier (deprecaded, nutzt discinfo_get_identifier())
# R�ckgabe.: Anzahl der Fehlversuche (0-N) via echo
# .........  Return-Code: 0 = Erfolg (Anzahl ermittelt)
# .........  ............ 1 = Fehler (Datei nicht verf�gbar, Fallback auf 0)
...
common_get_disc_failure_count() {
    ...
    #-- Pr�fe ob Eintrag vorhanden ist --------------------------------------
    if [[ -n "$value" ]]; then
...
# Parameter: $1 = Disc-Identifier (deprecaded, nutzt discinfo_get_identifier())
# R�ckgabe.: 0 = Erfolg (Fehler registriert)
# .........  1 = Fehler (Datei nicht verf�gbar)
...
# Parameter: keine (nutzt DISC_INFO Array)
# R�ckgabe.: 0 = Erfolg (Eintrag gel�scht oder nicht vorhanden)
# .........  1 = Fehler (Datei nicht verf�gbar)
# Extras...: Loggt nur wenn tats�chlich ein Eintrag gel�scht wurde
# .........  Nutzt get_ini_value zur Existenzpr�fung vor dem L�schen
# ===========================================================================
common_clear_disc_failures() {
    #-- Debug-Log Eintrag ---------------------------------------------------
...
# ---------------------------------------------------------------------------
# Funktion.: Berechnet und loggt Kopierfortschritt (zentral f�r alle Methoden)
# .........  Sendet Updates an API, MQTT und systemd-notify
# Parameter: $1 = Aktuell kopierte Bytes
# .........  $2 = Gesamtgr��e in Bytes (0 = unbekannt)
# .........  $3 = Start-Zeit (Unix-Timestamp)
# .........  $4 = Log-Pr�fix (z.B. "DATA", "DVD", "BLURAY")
# R�ckgabe.: keine (setzt globale Variablen $percent und $eta)
...
common_calculate_and_log_progress() {
    ...
    # Berechne Prozent und ETA wenn m�glich
    if [[ $total_bytes -gt 0 ]] && [[ $current_bytes -gt 0 ]]; then
        ...
        # systemd-notify: Status aktualisieren (wenn verf�gbar)
...
# ---------------------------------------------------------------------------
# Funktion.: �berwacht Kopierfortschritt f�r dd/ddrescue im Hintergrund
# .........  Nutzt zeitgesteuertes Monitoring (alle 60 Sekunden)
# Parameter: $1 = PID des Kopierprozesses
# .........  $2 = Gesamtgr��e in Bytes (f�r Prozentberechnung)
# .........  $3 = ISO-Dateiname (zur Gr��enermittlung via stat)
# R�ckgabe.: keine (blockiert bis Prozess beendet ist)
# Extras...: Nutzt common_calculate_and_log_progress() f�r Fortschrittsberechnung
# .........  Konsistent mit Web-UI (60 Sekunden Intervall)
# .........  Macht KEIN wait - aufrufende Funktion muss wait ausf�hren!
# ===========================================================================
common_monitor_copy_progress() {
    local copy_pid=$1
    local total_bytes=$2
...
# .........  (Container-aware mit LXC-Unterst�tzung)
...
# R�ckgabe.: 0 = Erfolg (Disc ausgeworfen)
# .........  1 = Fehler (Device nicht verf�gbar oder Eject fehlgeschlagen)
# Extras...: Nutzt wait_for_medium_change() f�r Container-Umgebungen
...
# ---------------------------------------------------------------------------
# Funktion.: R�umt Ressourcen nach Disc-Operation auf (Temp-Verzeichnis,
# .........  unvollst�ndige ISO-Dateien, DISC_INFO Reset)
# Parameter: $1 = Status (optional: "success", "failure", "unknown", "interrupted")
# .........  Falls nicht �bergeben, wird Status aus Fehler-Tracking ermittelt
# R�ckgabe.: keine
# Extras...: Wirft Disc NICHT aus (siehe common_eject_and_wait)
# .........  Unmountet Temp-Verzeichnisse automatisch
# .........  L�scht ISO-Dateien nur bei Status "failure"
# Hinweis..: ISO-Existenz ist KEIN Erfolgsindikator (unvollst�ndige Dateien!)
# ===========================================================================
common_cleanup_disc_operation() {
    local status="${1}"
    local temp_dir iso_file
    
    # Ermittle Status automatisch wenn nicht �bergeben
    if [[ -z "$status" ]]; then
    ...
    # 1. Temp-Verzeichnis aufr�umen (falls vorhanden)
    ...
    if [[ -n "$temp_dir" ]] && [[ -d "$temp_dir" ]]; then
        ...
        # L�sche Temp-Verzeichnis (mit force)
    ...
    # 2. Unvollst�ndige ISO-Datei l�schen (nur bei Fehler)
    if [[ "$status" == "failure" ]]; then
        iso_file=$(discinfo_get_iso_filename)
        [[ -n "$iso_file" ]] && [[ -f "$iso_file" ]] && rm -f "$iso_file"
    fi
    
    # 3. Variablen zur�cksetzen (immer)
    discinfo_init
}
...
# ---------------------------------------------------------------------------
# Funktion.: Sammelt Informationen �ber installierte Core-Software
# Parameter: keine
# R�ckgabe.: Schreibt JSON-Datei mit Software-Informationen
# ===========================================================================
common_collect_software_info() {
    ...
    if type -t systeminfo_check_software_list &>/dev/null; then
        ...
        log_error "COMMON: systeminfo_check_software_list nicht verf�gbar"
        return 1
    fi
}
...
# ---------------------------------------------------------------------------
# Funktion.: Gibt Software-Informationen als JSON zur�ck
# Parameter: keine
# R�ckgabe.: JSON-String mit Software-Informationen
# ===========================================================================
common_get_software_info() {
    local cache_file
